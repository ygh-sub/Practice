...
const MealForm = ({ onSubmit, defaultDate }) => {
  ...
  const handleChange = (e) => {
    ...
    // Reset estimation status when user changes calories manually
    if (name === 'calories') {
  ...
  const handleEstimateCalories = async () => {
    ...
    try {
      ...
      // Show explanation to user if available
      if (result.explanation) {
        setEstimationError(result.explanation); // Using error state for info message
      }
    } catch (error) {
  ...
  const handleSubmit = (e) => {
    ...
    // Allow submission without calories (will be 0 or estimated)
